...
function checkForm(){
  ...
  <body>
  	<c:if test="${empty sessionScope.ShowPermission}">
      <script>location="/back/permission.action";</script>
    </c:if>
    <form method="post" name="form1" action="/back/permission!BatchDeletePermission.action" onsubmit="return checkForm();">
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td width="8" rowspan="3" background="/back/images/shadow.gif">
    </td>
    <td width="943" height="30" valign="middle" background="/back/images/table_bg001.gif"> &nbsp;&nbsp; <img src="/back/images/b_sing.gif" width="12" height="12" align="middle" />&nbsp;<font color="#999999">&nbsp;&nbsp; 提示信息及功能说明，总共： ${sessionScope.ShowPermission.totalRows } 条数据</font></td>
  </tr>
   <!----------------------表格标题部分------------------------------>
  <tr>
    ...
    <table width="100%" border="0" cellspacing="1" cellpadding="0" align="center">
      ...
      <!----------------------动态数据显示部分------------------------------>
      <c:forEach items="${sessionScope.ShowPermission.data }" var="per">
      <tr height="30px" bgcolor="#F7F7F7" 
        ...
        <td align="center" valign="middle"><a href="/back/permission!UpdatePermission.action?perid=${per.perid}">更改</a>&nbsp;&nbsp; 
        		<a href="javascript:if(confirm('确定删除该条记录吗？')==true){location='/back/permission!DeletePermission.action?perid=${per.perid}';}">删除</a></td>
      </tr>
      </c:forEach>
      ...
      <tr height="35px">
        <td height="40" colspan="3" align="left" valign="middle">
        	当前第： ${sessionScope.ShowPermission.currentPage } / ${sessionScope.ShowPermission.totalPages } 页&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
   ...
   <c:forEach var="i" begin="1"
							end="${sessionScope.ShowPermission.totalPages }">
							<c:if test="${sessionScope.ShowPermission.currentPage == i}"> 【${i}】 </c:if>
							<c:if test="${sessionScope.ShowPermission.currentPage != i}">
								<a href="/back/permission.action?currentPage=${i}">${i}&nbsp;</a>
							</c:if>
						</c:forEach>    
       页</td>
        </tr>
